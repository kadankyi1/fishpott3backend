...
class AdministratorController extends Controller
{
    ...
    public function searchOrders(Request $request)
    {
        ...
        */
        ...
        if(empty($request->keyword)){
            ...
            ->orWhere('stock_purchases.stockpurchase_sys_id', $request->keyword)
            //->where('stockpurchase_payment_gateway_status', '!=', 0)
        ...
        if(empty($request->keyword)){
            ...
            ->join('countries', 'businesses.business_country_id', '=', 'countries.country_id')
            ->where('user_phone_number', 'LIKE', "%{$request->keyword}%")
            ->orWhere('stocks_transfers.stocktransfer_sys_id', $request->keyword)
            //->where('stockstransfers_processed', '=', 0)
            //->where('stocktransfer_payment_gateway_status', '!=', 0)
        ...
        if(empty($request->keyword)){
            ...
            ->join('countries', 'businesses.business_country_id', '=', 'countries.country_id')
            ->where('user_phone_number', 'LIKE', "%{$request->keyword}%")
            ->orWhere('stock_sell_backs.stocksellback_sys_id', $request->keyword)
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |--------------------------------------------------------------------------
    | THIS FUNCTION ADDS A NEW SHARE VALUE
    |--------------------------------------------------------------------------
    |--------------------------------------------------------------------------
    */
    ...
    */
    ...
    public function updateOrderProcessedOrFlaggedStatus(Request $request)
    {
        ...
        //UtilController::notifyOneUserAndEmail($request->order_id, "Order Processed", "Your stock transfer order with ID TEST92111. processed successfully"); return;
        /*
        |**************************************************************************
        | VALIDATION STARTS 
        |**************************************************************************
