...
class UtilController extends Controller
{
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
    public static function pottnameIsTaken($keyword)
    {
        $user = User::where('user_pottname', '=', $keyword)->first();
        if ($user === null) {
            return false;
    ...
    */
    ...
    public static function getLatestSuggestion()
    {
        ...
        //if ($suggestion != null && UtilController::getDateDiff($suggestion->created_at, date('Y-m-d H:i:s'), "minutes") >= intval(5)) {
                return false;
        } 
        ...
        foreach($users as $user){
            ...
            if($suggestion_given != 1){
                ...
                UtilController::sendNotificationToUser(
                    ...
                    "suggestion-info",
                    "Stock Suggestion - FishPott",
                    "You got no business suggestion. Your pott is not smart enough.",
                    "", 
                    "", 
                    "", 
                    "", 
